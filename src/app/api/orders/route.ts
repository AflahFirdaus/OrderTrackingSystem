...
export async function GET(request: Request) {
  // Wrap entire function in try-catch to ensure JSON response
  try {
    ...
    let orders: Order[] = [];
    try {
      orders = await query<Order[]>(sql, params);
    } catch (dbError: any) {
      console.error("Database query error:", dbError);
      // Return JSON error instead of letting it propagate
      return NextResponse.json(
        { 
          error: "Database error",
          details: dbError.message || "Failed to fetch orders",
          hint: dbError.message?.includes("Missing MySQL") 
            ? "Buat file .env.local di root project dengan MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, dan MYSQL_DATABASE"
    ...
    try {
      ...
      // Return orders without items if processing fails
    ...
    // Always return JSON, never let error propagate to Next.js error page
    const errorMessage = error?.message || "Unknown error";
    
    if (errorMessage.includes("Missing MySQL") || errorMessage.includes("Missing DATABASE")) {
      return NextResponse.json(
        { 
          error: errorMessage,
